...
const help = `omo-tmux-dag — install the OmO DAG viewer for tmux

Usage:
  omo-tmux-dag install [--dry-run] [--agent-dir PATH] [--lang en|ko]
  omo-tmux-dag --help
  omo-tmux-dag --version

Install OmO and tmux separately. After installation, start OmO in a tmux
pane and run /reload in an existing session.
...
try {
  ...
  console.error(`omo-tmux-dag: ${error.message}`);
  process.exitCode = 1;
}
