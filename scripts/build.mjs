...
try {
  ...
  await cp(join(root, 'NOTICE'), join(stage, 'NOTICE'));
  await cp(join(root, 'LICENSE.upstream'), join(stage, 'LICENSE.upstream'));
  ...
  execFileSync(process.execPath, ['--check', join(root, 'bin/omo-tmux-dag.mjs')], { stdio: 'pipe' });
  // Replace only the generated output after all source files pass syntax checks.
  await rm(join(root, 'dist'), { recursive: true, force: true });
  await rename(stage, join(root, 'dist'));
  console.log('Built dist/: extension, viewer, installer, MIT license, and NOTICE. JavaScript syntax checks passed.');
} finally {
