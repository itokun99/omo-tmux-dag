...
const container = join(agentDir, 'tmux-dag', 'integration');
const wrapper = join(agentDir, 'extensions', 'tmux-dag.js');
const marker = '// managed by omo-tmux-dag';
...
try {
  const currentFile = await readFile(wrapper, 'utf8');
  if (!currentFile.startsWith(marker)) throw new Error(t(language, 'existingFile', { path: wrapper }));
} catch (error) { if (error.code !== 'ENOENT') throw error; }
...
const plan = { integration, extension: wrapper, source, entry: 'tmux-dag.js', language,
...
try {
  ...
  await cp(join(source, 'NOTICE'), join(staged, 'NOTICE'));
  await cp(join(source, 'LICENSE.upstream'), join(staged, 'LICENSE.upstream'));
  ...
  const text = `${marker}\nexport { default } from '../tmux-dag/integration/${generation}/extension.mjs';\n`;
