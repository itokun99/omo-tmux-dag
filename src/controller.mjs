...
import { isMissingPaneError, shellCommand } from './tmux.mjs';
...
export function isDagViewerPane(pane) {
  ...
  return typeof title === 'string' && (title.startsWith('DAG · ') || title === 'OmO DAG');
}
...
export class DagPane {
  constructor({ sessionId, parentPane, socket, stateDir, cwd, node, viewer, tmux, notify = () => {}, language = 'en', taskStateDir }) {
    Object.assign(this, { sessionId, parentPane, cwd, node, viewer, tmux, notify });
  ...
  async listDagPanes() {
    try {
      const window = await this.tmux.windowOf(this.parentPane);
      const panes = await this.tmux.list(window);
      return panes.filter(isDagViewerPane)
        .filter(pane => pane.pane_id && pane.pane_id !== this.parentPane)
  ...
  async closePane(paneId) {
    try { await this.tmux.close(paneId); }
    catch (error) {
  ...
  async ensure(force) {
    ...
    if (record?.paneId) {
      try {
        await this.tmux.windowOf(record.paneId);
        if (record.ready) {
    ...
    const paneId = await this.tmux.split({ parentPane: this.parentPane, cwd: this.cwd,
      command: shellCommand([this.node, this.viewer, '--state', this.stateFile]) });
    if (!paneId) throw new Error(t(this.language, 'missingPaneId'));
    await writeJson(this.recordFile, { paneId, ready: false });
    await this.tmux.rename(paneId, dagTitle(this.sessionId));
