...
const probe = 'JSON.stringify({runtime:"omo-tmux-dag-node",version:process.versions.node,bun:!!process.versions.bun,path:process.execPath})';

export async function resolveViewerNode({ env = process.env, execPath = process.execPath, run = execute, language = 'en' } = {}) {
  const override = env.OMO_TMUX_DAG_NODE?.trim();
  ...
  for (const candidate of new Set(candidates)) {
    try {
      ...
      if (result.runtime === 'omo-tmux-dag-node' && !result.bun &&
