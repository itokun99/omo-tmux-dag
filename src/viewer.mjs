...
import { closePane } from './tmux.mjs';
...
function setTitle() {
  const name = state?.sessionId ? `DAG · ${String(state.sessionId).slice(0, 8)}` : 'OmO DAG';
...
async function close(closeSelf = false) {
  if (closing) return;
  closing = true;
  ...
  if (closeSelf) {
    const pane = (process.argv.includes('--close-pane') ? process.argv[process.argv.indexOf('--close-pane') + 1] : undefined)
